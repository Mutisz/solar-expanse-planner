import type { GroundFacility } from '../types';
import {
  AmountInput,
  FavoriteButton,
  ResourceCells,
...
export default function GroundFacilitiesTable({ data, amounts, onAmountChange, favorites, onFavoriteToggle, onResetAmounts }: Props) {
      ...
      <table className={tableClass}>
        <thead className="bg-gray-900">
          <tr>
            ...
            <th className={thClass}>Terraforming</th>
        ...
        <tbody>
          {sorted.map((item) => (
            <tr key={item.name} className="hover:bg-gray-900/50">
              ...
              <td className={tdClass}>{item.terraforming || '—'}</td>

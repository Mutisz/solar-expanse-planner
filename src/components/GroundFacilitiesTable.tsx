...
const COLUMN_DEFS: ColumnDef[] = [
  ...
  { key: 'terraforming',  label: 'Terraforming',      defaultOn: false },
...
export default function GroundFacilitiesTable({ data, amounts, onAmountChange, favorites, onFavoriteToggle, onResetAllAmounts, onResetAmounts }: Props) {
  const resources = resourcesForDataset(data);
  const sorted = sortWithFavorites(data, favorites);
  const [visible, setVisible] = useLocalStorage('ground-facilities-cols', defaultVisible(COLUMN_DEFS));
